...
class CompleteLogger:
    ...
    def __init__(self, root, phase='train', args=None):
        self.root = root
        self.phase = phase
        ...
        if args.data=='Retail' and 'test' in args.phase:
            assert args.test_difficulty in ['val', '1', '2', '3'], 'Please check args.test_difficulty'
            log_filename = os.path.join(self.root, "{}-difficulty{}-{}.txt".format(phase, args.test_difficulty, now))
        else:
            log_filename = os.path.join(self.root, "{}-{}.txt".format(phase, now))
