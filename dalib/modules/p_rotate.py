...
class P_Rotate_single_img(object):
    ...
    def __call__(self, img):    # img must be a tensor whose shape has the form of (C, H, W).
        ...
        for patch in patches:
            ...
            rotated_patch = rotate(patch, rotate_angle)
            rotated_patches.append(rotated_patch)
        
        for idx, patch in enumerate(rotated_patches):
...
class P_Selective_Rotate_single_img(object):
    ...
    def __call__(self, img):    # img must be a tensor whose shape has the form of (C, H, W).
        ...
        idx_select = sorted(random.sample(range(n_patch),n_select))
        
        for idx in idx_select:
            selected_patch = patches[idx]
            rotate_angle = random.choice(self.angles)
            rotated_patch = rotate(selected_patch, rotate_angle)
            patches[idx] = rotated_patch
...
class P_Rotate(nn.Module):
    def __init__(self, patch_size):
        super(P_Rotate, self).__init__()
...
class P_Selective_Rotate(nn.Module):
    def __init__(self, patch_size, ratio):
        super(P_Selective_Rotate, self).__init__()

        self.patch_size = patch_size
        self.ratio = ratio
        self.rotate = P_Selective_Rotate_single_img(self.patch_size, self.ratio)
...
if __name__=='__main__':
    ...
    if not args.selective:
        transform = transforms.Compose([transforms.PILToTensor(),
                                        ...
                                        P_Rotate_single_img(args.patch_size),
                                        transforms.ToPILImage()])
    else:
        transform = transforms.Compose([transforms.PILToTensor(),
                                        transforms.Resize((args.resize, args.resize)),
                                        P_Selective_Rotate_single_img(args.patch_size, args.ratio),
                                        transforms.ToPILImage()])
