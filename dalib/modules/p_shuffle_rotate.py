...
class P_Shuffle_Rotate_single_img(object):
    ...
    def __call__(self, img):    # img must be a tensor whose shape has the form of (C, H, W).
        ...
        selected_patch_rand_order = []
        for i, idx_rand_order in enumerate(idx_select_rand_order):
            rotate_angle = random.choice(self.angles)
            selected_patch_rand_order.append(rotate(patches[idx_rand_order], rotate_angle))
...
class P_Shuffle_Rotate(nn.Module):
    def __init__(self, patch_size, ratio):
        super(P_Shuffle_Rotate, self).__init__()

        self.patch_size = patch_size
        self.ratio = ratio
        self.shuffle_rotate = P_Shuffle_Rotate_single_img(self.patch_size, self.ratio)
    ...
    def forward(self, img: Tensor):
        ...
        for i, single_img in enumerate(img):
            neg_aug_img = self.shuffle_rotate(single_img)
...
if __name__=='__main__':
    ...
    transform = transforms.Compose([transforms.PILToTensor(),
                                    ...
                                    P_Shuffle_Rotate_single_img(args.patch_size, args.ratio),
