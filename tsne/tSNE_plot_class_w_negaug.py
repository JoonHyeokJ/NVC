...
from dalib.modules import p_shuffle
...
def create_sdat_feature_encoder(args, num_classes):
    ...
    # print(backbone)
    pool_layer = nn.Identity() if args.no_pool else None  # args.no_pool is usually True when using ViT.
...
if __name__ == '__main__':
    ...
    parser.add_argument("--with_legend", action='store_true', help="Do you want for the resultant figure to have the legend?")
    ...
    negative_aug = p_shuffle.P_Shuffle(patch_size=32)

    features=None
    features_neg_view=None
    labels=None
    labels_neg_view=None
    ...
    for j, batch_t in enumerate(tqdm(target_loader, desc='Running the model inference in target_loader...')):
        ...
        neg_view_t = negative_aug(image_t)
        with torch.no_grad():
            output = feature_encoder(image_t)
            output_neg_view = feature_encoder(neg_view_t)
        current_outputs_t = output.to('cpu').detach().numpy()
        current_outputs_t_neg_view = output_neg_view.to('cpu').detach().numpy()
        features = np.concatenate((features, current_outputs_t))
        labels = np.concatenate((labels, np.array(current_label_t)))
        if j==0:
            features_neg_view = current_outputs_t_neg_view
            labels_neg_view = np.array(current_label_t)
        else:
            features_neg_view = np.concatenate((features_neg_view, current_outputs_t_neg_view))
            labels_neg_view = np.concatenate((labels_neg_view, np.array(current_label_t)))
    num_target_samples = len(labels) - num_source_samples
    assert num_target_samples == len(labels_neg_view)

    all_features = np.concatenate((features, features_neg_view))

    print('Running TSNE...')
    tsne = TSNE(n_components=2).fit_transform(all_features)
    ...
    target_labels = labels[num_source_samples:num_source_samples+num_target_samples]

    target_tx_neg_view = tx[num_source_samples+num_target_samples:]
    target_ty_neg_view = ty[num_source_samples+num_target_samples:]
    target_labels_neg_view = labels_neg_view

    # breakpoint()
    # plotting source domain samples
    for label in colors_per_class:
        indices = [i for i, lbl in enumerate(source_labels) if lbl  == label]

        current_tx = np.take(source_tx, indices)
        current_ty = np.take(source_ty, indices)

        color = colors_per_class[label]

        ax.scatter(current_tx, current_ty, c=color, label=label, marker='o', s=3, edgecolors='0', linewidths=0.1)
    
    # plotting target domain samples
    for label in colors_per_class:
        indices = [i for i, lbl in enumerate(target_labels) if lbl  == label]

        current_tx = np.take(target_tx, indices)
        current_ty = np.take(target_ty, indices)

        color = colors_per_class[label]

        ax.scatter(current_tx, current_ty, c=color, label=label, marker='X', s=9, edgecolors='0', linewidths=0.1)
        # When it comes to marker, please refer to https://matplotlib.org/stable/api/markers_api.html#module-matplotlib.markers

    # plotting target domain negative views
    for label in colors_per_class:
        indices = [i for i, lbl in enumerate(target_labels) if lbl  == label]

        current_tx = np.take(target_tx_neg_view, indices)
        current_ty = np.take(target_ty_neg_view, indices)

        color = colors_per_class[label]

        ax.scatter(current_tx, current_ty, c=color, label=label, marker='^', s=9, edgecolors='0', linewidths=0.1)
        # When it comes to marker, please refer to https://matplotlib.org/stable/api/markers_api.html#module-matplotlib.markers
    ...
    if args.with_legend:
        legend = ax.legend(fontsize='xx-small', loc='upper center', ncol=12, bbox_to_anchor=(0.5, -0.1))
    ...
    target_fig_name = 'plot_class_w_negaug.jpg' if args.result_path == 'near_model' else os.path.basename(args.result_path)
    ...
    if args.with_legend:
        plt.tight_layout()
        plt.savefig(final_target_path, dpi=400, bboxinches='tight', bbox_extra_artists=(legend,))
    else:
